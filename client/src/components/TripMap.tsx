...
type TripMapProps = {
  ...
  onMobileDrivingStateChange?: (isActive: boolean) => void;
  onSearchFocus: () => void;
};
...
const MOBILE_DIRECTIONS_MEDIA_QUERY = "(max-width: 720px)";
...
export const TripMap = ({
  ...
  const isDirectionsCameraDetachedRef = useRef(false);
  ...
  const setDirectionsCameraDetachedState = useCallback((value: boolean) => {
    isDirectionsCameraDetachedRef.current = value;
  }, []);
  ...
  const drawDirectionsRoute = useCallback(
    (coordinates: [number, number][], travelMode: "driving" | "walking") => {
      const map = mapRef.current;
      if (!map) {
        return;
      }
      ...
      const source = map.getSource(DIRECTIONS_SOURCE_ID) as
        | GeoJSONSource
        | undefined;
      if (source) {
  ...
  const focusMapOnCoordinates = useCallback(
    async (longitude: number, latitude: number): Promise<void> => {
      const map = mapRef.current;
      if (!map) {
        return;
      }

      const [minLng, minLat, maxLng, maxLat] = await buildFocusBounds(
        longitude,
        latitude,
      );
  ...
  const followDirectionsCamera = useCallback(
    (origin: [number, number], destination: [number, number]) => {
      ...
      if (
        ...
        isDirectionsCameraDetachedRef.current
      ) {
        return;
      }

      const cameraPadding =
        directionsTarget?.travelMode === "driving" &&
        window.matchMedia(MOBILE_DIRECTIONS_MEDIA_QUERY).matches
          ? MOBILE_DRIVING_CAMERA_PADDING
          : DRIVING_CAMERA_PADDING;

      const routeBearing = resolveRouteForwardBearing(
        origin,
      ...
      const bearing =
        routeBearing ??
        currentUserHeadingRef.current ??
        calculateBearingDegrees(origin, destination);
    ...
    [directionsTarget?.travelMode, isMapLoaded],
  );

  const recenterDirectionsCamera = useCallback(() => {
    if (!directionsTarget || !currentUserCoordinatesRef.current) {
      return;
    }
    ...
    setDirectionsCameraDetachedState(false);
    followDirectionsCamera(currentUserCoordinatesRef.current, [
    ...
    setDirectionsCameraDetachedState,
  ]);
    ...
    async (query: string) => {
      ...
      const requestSearch = async (
        ...
        const response = await fetch(
          buildRequestUrl(useCategoryEndpoint, useViewportBounds),
          {
      ...
      try {
        ...
        setSearchError(
          nextResults.length === 0 ? "No results found for this search." : null,
        );
      } catch (error) {
  ...
  useEffect(() => {
    ...
    directionsTarget?.locationId,
    directionsTarget?.travelMode,
    setDirectionsCameraDetachedState,
  ]);
  ...
  useEffect(() => {
    ...
    const updateUserLocation = (position: GeolocationPosition) => {
      ...
      if (reportedHeading !== null) {
        ...
        currentUserHeadingRef.current = calculateBearingDegrees(
          previousCoordinates,
          nextCoordinates,
        );
      }
      ...
      if (!userLocationMarkerRef.current) {
        ...
        userLocationMarkerRef.current = new mapbox.Marker({
          element: markerElement,
          anchor: "center",
        })
          .setLngLat([lng, lat])
  ...
  useEffect(() => {
    ...
    const markUserInteraction = () => {
      if (hasBegunDirections) {
        ...
        setDirectionsCameraDetachedState(true);
  ...
  }, [hasBegunDirections, isMapLoaded, setDirectionsCameraDetachedState]);
    ...
    locations.forEach((location) => {
      ...
      if (existing) {
        ...
        const label = existing
          .getElement()
          .querySelector<HTMLElement>(".map-location-label");
        if (label) {
      ...
      const marker = new mapbox.Marker({
        element: createLocationMarkerElement(location),
        anchor: "bottom",
      })
        .setLngLat(lngLat)
        .setPopup(popup)
      ...
      markerElement.setAttribute(
        "aria-label",
        `Open TODO list for ${location.name}`,
      );
  ...
  useEffect(() => {
    ...
    followDirectionsCamera(userCoordinates, [
      directionsTarget.longitude,
      directionsTarget.latitude,
    ]);
  }, [
  ...
  useEffect(() => {
    ...
    if (!directionsTarget) {
      const shouldResetCamera =
        hasDirectionsCameraOverrideRef.current ||
        activeDirectionsKeyRef.current !== null;
    ...
    if (
      ...
      distanceBetweenCoordinatesInMeters(lastFetch.origin, userCoordinates) <
        ROUTE_FETCH_MOVE_THRESHOLD_METERS
    ) {
      return;
    }
    ...
    const isRecalculatingDirections =
      directionsTarget.travelMode === "driving" &&
      hasBegunDirections &&
    ...
    setDirectionsState({
      status: "loading",
      destinationName: directionsTarget.name,
      message: isRecalculatingDirections
        ? "Re-calculating directions..."
        : `Calculating ${directionsTarget.travelMode} directions...`,
    });
    ...
    const directionsProfile =
      directionsTarget.travelMode === "walking" ? "walking" : "driving";
    const url = new URL(
    ...
    void fetch(url, { signal: controller.signal })
      ...
      .then((data) => {
        ...
        const nextStepInstruction =
          route?.legs?.[0]?.steps?.[0]?.maneuver?.instruction?.trim() ||
          "Head toward your destination.";
        if (!route || !coordinates || coordinates.length === 0) {
          throw new Error(
            data.message ?? "No drivable route found for this destination.",
          );
        }
        ...
        setDirectionsState({
          ...
          arrivalTimeLabel: formatArrivalTime(route.duration / 60),
          nextStepInstruction,
        });
  ...
  const isMobileDrivingDirections =
    hasBegunDirections && directionsTarget?.travelMode === "driving";

  useEffect(() => {
    onMobileDrivingStateChange?.(Boolean(isMobileDrivingDirections));

    return () => {
      onMobileDrivingStateChange?.(false);
    };
  }, [isMobileDrivingDirections, onMobileDrivingStateChange]);

  return (
    <section
      className={`map-stage ${isMobileDrivingDirections ? "is-mobile-driving-directions" : ""}`}
      aria-label="Trip map"
    >
      <div ref={mapContainerRef} className="map-canvas" />
      {accessToken && !isMapLoaded ? (
      ...
      <div className="map-search-control">
        ...
        {searchQuery.trim().length >= 2 ? (
          ...
          >
            {isSearchLoading ? (
              <p className="map-search-status">Searching this map view...</p>
            ) : null}
      ...
      <button
        ...
        onClick={
          hasBegunDirections ? recenterDirectionsCamera : focusMapOnUserLocation
        }
        disabled={!hasUserLocation}
        aria-label={
          hasBegunDirections
            ? "Re-center on my position and bearing"
            : "Focus my location"
        }
        title={
          hasBegunDirections
      ...
      {directionsTarget ? (
        <section className="map-directions-panel" aria-live="polite">
          ...
          {directionsState.status === "ready" ? (
            <div className="map-directions-layout">
              <div className="map-directions-step-block">
                <span className="map-directions-label">Next step</span>
                <strong className="map-directions-step-copy">
                  {directionsState.nextStepInstruction}
                </strong>
              </div>
              <div className="map-directions-summary-block">
                <div className="map-directions-summary-row">
                  <span className="map-directions-label">Miles left:</span>
                  <strong>
                    {formatDistanceMiles(directionsState.distanceMiles)}
                  </strong>
                </div>
                <div className="map-directions-summary-row">
                ...
                <div className="map-directions-summary-row">
                  <span className="map-directions-label">Arrival</span>
                  <strong>{directionsState.arrivalTimeLabel}</strong>
                </div>
              </div>
            </div>
          ) : null}
          ...
          <div className="map-directions-actions">
            ...
            {directionsState.status === "ready" && hasBegunDirections ? (
              <button
                ...
                onClick={recenterDirectionsCamera}
              >
                Re-center
              </button>
            ) : null}
      ...
      {!accessToken ? (
        <div className="map-warning">
          Set VITE_MAPBOX_ACCESS_TOKEN to render the map background.
        </div>
      ) : null}
    </section>
  );
